...
contract KIP17 is KIP13, IKIP17 {
    ...
    mapping (address=>mapping(uint256=>uint256)) private _tokenTimestamp;
    
    mapping (address=>mapping(uint256=>uint8)) private _courseNum;
    /*
    ...
    // function tokenTimestamp(address account,uint256 tokenId) public view returns(uint256){
    //     return _tokenTimestamp[account][tokenId];
    // }
    
    function certData(address account,uint256 tokenId) public view returns(uint8,uint256){
        return (_courseNum[account][tokenId],_tokenTimestamp[account][tokenId]);
    }
    
    function mintCert(address to,uint8 num ,uint256 tokenId) public returns (bool) {
        require(num<7 && num>=0);
        _mint(to, num,tokenId);
        return true;
    }
     ...
     */
    function _mint(address to,uint8 num, uint256 tokenId) internal {
        require(to != address(0), "KIP17: mint to the zero address");
        require(!_exists(tokenId), "KIP17: token already minted");

        _tokenOwner[tokenId] = to;
        _ownedTokensCount[to].increment();
        _tokenTimestamp[to][tokenId]=now;
        _courseNum[to][tokenId]=num;
